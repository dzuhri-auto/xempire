...
def get_tele_user_obj_from_query_id(query_id):
    ...
    hash = query_obj.get("hash")
    return tele_user_obj, hash


def set_sign_headers(data: dict) -> None:
    time_string = str(int(time.time()))
    json_string = json.dumps(data)
    hash_object = hashlib.md5()
    hash_object.update(f"{time_string}_{json_string}".encode("utf-8"))
    hash_string = hash_object.hexdigest()
    return time_string, hash_string
...
def improve_possible(
    skill: dict, my_skills: dict | list, level: int, balance: int, friends: int
) -> dict | None:
    possible = False
    my_skill = None
    if isinstance(my_skills, dict) and skill["key"] in my_skills:
        my_skill = my_skills[skill["key"]]
        if skill["maxLevel"] <= my_skill["level"]:
            return None
        if isinstance(my_skill["finishUpgradeDate"], str):
            my_skill["finishUpgradeDate"] = (
                datetime.strptime(my_skill["finishUpgradeDate"], "%Y-%m-%d %H:%M:%S")
                .replace(tzinfo=timezone.utc)
                .timestamp()
            )
        if (
            isinstance(my_skill["finishUpgradeDate"], (int, float))
            and my_skill["finishUpgradeDate"] > time.time()
        ):
            return None
    ...
    if balance > skill_price:
        if not skill["levels"]:
            ...
            if my_skill is not None:
                ...
                matched_skill_limit = (
                    skill["levels"][0] if skill["levels"][0]["level"] == 1 else None
                )
            if matched_skill_limit is None:
                possible = True
            elif (
                matched_skill_limit["requiredHeroLevel"] <= level
                and matched_skill_limit["requiredFriends"] <= friends
            ):
                if not matched_skill_limit["requiredSkills"]:
                    possible = True
                else:
                    for req_skill, req_level in matched_skill_limit["requiredSkills"].items():
                        if isinstance(my_skills, dict):
                            if my_skills.get(req_skill, {}).get("level", 0) >= req_level:
                                possible = True
    ...
    if possible:
        ...
        if isinstance(my_skills, dict):
            skill["newlevel"] = (
                my_skills.get(skill["key"], {}).get("level", 0) + 1
            )  # new level for improve or 1 level for buy
        else:
...
def calculate_best_skill(
    skills: list,
    ignored_skills: list,
    profile: dict,
    level: int,
    balance: int,
    improve: dict | list | None,
) -> dict | None:
    ...
    if isinstance(my_skills, dict):
        for my_skill, my_limit in my_skills.items():
            if type(my_limit["finishUpgradeDate"]) is str:
                my_skills[my_skill]["finishUpgradeDate"] = (
                    datetime.strptime(my_limit["finishUpgradeDate"], "%Y-%m-%d %H:%M:%S")
                    .replace(tzinfo=timezone.utc)
                    .timestamp()
                )
    ...
    possible_skills = []
    for skill in skills:
        if skill["key"] in ignored_skills:
            continue
        if skill["profitBasic"] == 0:
            continue
    ...
    if possible_skills:
        match settings.SKILLS_MODE:
            case "profitness":
                possible_skills = sorted(
                    possible_skills,
                    key=lambda x: (x["profit"] / x["price"], x["profit"]),
                    reverse=True,
                )
            case "profit":
        ...
        #         print(
        #             f"{skill['key']:35} | price: {number_short(skill['price']):>7} | profit: {number_short(skill['profit']):>7}"
        #         )
